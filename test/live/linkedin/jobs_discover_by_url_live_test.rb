...
module BrightData
  module LinkedIn
    class JobsDiscoverByUrlLiveTest < Minitest::Test
      ...
      def test_scrape_returns_job_array
        jobs = live_results(
          name: "jobs_discover_by_url",
          scrape: -> { @client.linkedin.jobs.discover_by_url.scrape(urls: urls) },
          trigger: -> { @client.linkedin.jobs.discover_by_url.trigger(urls: urls) }
        )

        assert_live_results_present!("jobs_discover_by_url", jobs)
